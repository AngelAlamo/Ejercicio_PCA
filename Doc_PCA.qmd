...
library(ggplot2)
library("factoextra")
datos.acp=prcomp(datos, scale = FALSE)
...
Ojo, como teniamos los datos guardados, ya estaban centrados. Para conseguir los datos tipificados necesitamos los datos originales sin centrado y conseguiremos los veps y vaps de estos tipificados con la función `prcomp` especificando `scale = TRUE`
```{r}
datos_tip = datos + media
```



```{r PCA tipi}
datos_tip_pca = prcomp(datos_tip, scale = TRUE)
```
...
```{r}
autoplot(datos_tip_pca, data = datos_tip, loadings = TRUE, loadings.colour = 'blue',
