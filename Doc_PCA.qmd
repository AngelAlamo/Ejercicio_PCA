...
Consideramos los datos centrados, es decir, restamos a cada valor la media de la columna correspondiente.
...
S
```
Como los datos corresponden a los porcentajes de población, y por tanto, todas las variables están en una escala común y las diferencias entre las varianzas de las variables son informativas, el estudio utilizando la matriz de covarianzas es correcto. Si miramos la matriz cov-var, vemos que hay un rango muy alto de variablidad, por ejemplo la variable "Agr" presenta una variación de `cov(datos[,1])`, en cambio, variables como "Min", presenta una variación de ` cov(datos[,2])`, que es muy inferior. También hay variables con una varianza no tan extrema, como es el caso de la variable "Fab", con varianza `cov(datos[,3]` y "SSP".

```{r}
#No me acuerdo como se hacían chunks de línea xd
```
...
Una vez visto las matrices, pasamos al estudio de las componentes principales. Siguiendo el método visto en teoría, calculamos los valores y vectores propios de la matriz cov-var.
